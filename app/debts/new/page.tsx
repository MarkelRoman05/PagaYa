...
export default function NewDebt() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const normalizedAmount = formData.amount.replace(',', '.').replace(/[^0-9.-]/g, '');
          ...
          <Card className="shadow-lg border-none">
            ...
            <CardContent className="space-y-6">
                ...
                >
                  <Label
                    ...
                    className={cn(
                      ...
                      formData.type === 'owed_to_me' ? "border-primary bg-primary/5" : "border-muted hover:border-muted-foreground/30"
                    )}
                  >
                    <div className="flex flex-col gap-1">
                      <span className="font-bold">Me deben</span>
                      <span className="text-[10px] text-muted-foreground uppercase font-medium">Préstamo</span>
                    </div>
                    <RadioGroupItem value="owed_to_me" id="type-me" className="sr-only" />
                  </Label>
                  <Label
                    htmlFor="type-them"
                    className={cn(
                      "flex items-center justify-between rounded-xl border-2 p-4 cursor-pointer transition-all",
                      formData.type === 'owed_by_me' ? "border-orange-500 bg-orange-50" : "border-muted hover:border-muted-foreground/30"
                    )}
                  >
                    <div className="flex flex-col gap-1">
                      <span className="font-bold">Yo debo</span>
                      <span className="text-[10px] text-muted-foreground uppercase font-medium">Deuda</span>
                    </div>
                    <RadioGroupItem value="owed_by_me" id="type-them" className="sr-only" />
                  </Label>
                </RadioGroup>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.friendId} onValueChange={(val) => setFormData({ ...formData, friendId: val })}>
                ...
                {friends.length === 0 && (
                   <p className="text-xs text-orange-600 mt-1">Primero añade amigos en la sección de amigos.</p>
                )}
              </div>
            ...
            <CardFooter className="pt-2">
              <Button type="submit" disabled={friends.length === 0 || isSubmitting} className="w-full h-14 rounded-xl text-lg font-bold shadow-lg shadow-primary/20 disabled:shadow-none">
